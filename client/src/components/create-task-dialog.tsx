...
export function CreateTaskDialog({ defaultProjectId }: CreateTaskDialogProps) {
  ...
  const { setValue, getValues, handleSubmit, register, formState, reset, watch } = form;

  useEffect(() => {
    if (open) {
      if (defaultProjectId) {
         setValue("projectId", defaultProjectId);
      } else if ((!getValues("projectId") || getValues("projectId") === 0) && projects && projects.length > 0) {
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    createTask.mutate(payload, {
      ...
      onError: (error) => {
        ...
        // Try to log the detailed server response
        // @ts-ignore
        if (error.response) {
            // @ts-ignore
            console.error("Server Response Data:", error.response.data);
            // @ts-ignore
            toast.error(`Error: ${JSON.stringify(error.response.data.message || "Unknown error")}`);
        } else {
